...
import cn.cow.hotel.mapper.OccupancyRateMapper;
import cn.cow.hotel.response.AjaxResult;
import cn.cow.hotel.response.MsgType;
...
public class OpOrderController {
    ...
    @Autowired
    private OccupancyRateMapper occupancyRateMapper;
    ...
    @ApiOperation("获取入住率数据")
    @GetMapping("/occupancy-rate")
    public AjaxResult getOccupancyRateData() {
        try {
            List<OccupancyRate> occupancyRateData = occupancyRateMapper.getOccupancyRates();
            return ResponseTool.success(occupancyRateData);
        } catch (Exception e) {
            e.printStackTrace();
            return ResponseTool.failed("获取入住率数据失败");
        }
    }
